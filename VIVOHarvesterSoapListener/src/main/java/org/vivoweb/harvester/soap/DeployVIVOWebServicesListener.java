...
public class DeployVIVOWebServicesListener {
			...
			if (vivoSoapServer.initService(this.soapConfigPath)) {
				vivoSoapServer
						.deployRpcSoapService(
								org.vivoweb.harvester.soap.VIVOWebServicesListener.class,
								this.serviceName);
				vivoSoapServer.startServer();
				QSAdminServerConfig adminConfig = vivoSoapServer.getConfig()
	...
	private static ArgParser getParser() {
		...
		parser.addArgument(new ArgDef().setShortOption('d')
				.setLongOpt("decrypt").withParameter(true, "DECRYPT")
				.setDescription("The SEARCHMESSAGE file path.")
				.setRequired(true));
		parser.addArgument(new ArgDef().setShortOption('c')
				.setLongOpt("configFile").withParameter(true, "CONFIG")
				.setDescription("The location of jSoapServer config file")
				.setRequired(true));
		parser.addArgument(new ArgDef().setShortOption('o')
				.setLongOpt("output").withParameter(true, "OUTPUT_FOLDER")
				.setDescription("result file folder path").setRequired(true));
		parser.addArgument(new ArgDef().setShortOption('s')
				.setLongOpt("serviceName").withParameter(true, "SERVICE_NAME")
				.setDescription("The name of the published web service")
				.setRequired(true));
		...
		parser.addArgument(new ArgDef()
				.setShortOption('x')
				.setLongOpt("schemaFile")
				.withParameter(true, "SCHEMA_FILE")
				.setDescription(
						"This is the xsd file to compare your incoming xml against")
				.setRequired(false));
		parser.addArgument(new ArgDef().setShortOption('l')
